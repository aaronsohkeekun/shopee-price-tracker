...
def main():
    ...
    with st.expander("📸 **Track a New Price Entry**", expanded=True):
        ...
        if st.button("🔄 Clear All Fields & Start Over", key="clear_all"):
            # Reset every text field by clearing its key from session_state.
            # Streamlit re-draws the field as empty on the next rerun.
            for key in ["url_input", "manual_title", "manual_price",
                        "manual_seller", "manual_stock"]:
                if key in st.session_state:
                    del st.session_state[key]
            # Clear any pasted images too
        ...
        product_url = st.text_input(
            ...
            key="url_input",   # key= lets us clear this field programmatically
        )
        ...
        with oc1:
            # key= on every field allows the Clear button to reset them
        ...
        if analyze_clicked and not no_images:
            ...
            # Clear ALL fields after a successful save
            # so the form is clean and ready for the next product
            for key in ["url_input", "manual_title", "manual_price",
                        "manual_seller", "manual_stock"]:
                if key in st.session_state:
                    del st.session_state[key]
